...
namespace STVRogue.GameLogic
{
    ...
    public class TestPlayer : Player
    {
        ...
        {
            Command retCommand = new Command(1);
            int x = nextCommand.Count();
            if (x > 0)
            {
                foreach (Command command in nextCommand)
                {
                    retCommand = command;
                    nextCommand.Remove(retCommand);
                    break;
                }
            }
            else
            {
                //retCommand = base.getNextCommand();
                retCommand = new Command(4);
            }
            return retCommand;
        }
    }
    ...
    public class NTest_Dungeon
    {
        ...
        public void NTest_Fight()
        {
            ...
            Assert.IsTrue(node1.contested(testPlayer));
