...
{
    [DataContract(Name = "Pack", Namespace = "STVRogue.GameLogic")]
    public class Pack
    {
        [DataMember()]
        ...
        */
        public void Move(Node u)
        {
            ...
            if (location.level != u.level)
            {
                Logger.log("Pack " + id + " is trying to move to a node " + u.id + " from a different zone, this is not allowed. Rejected.");
                return;
            }
            //else move is successful
            this.location.packs.Remove(this);
            this.location = u;
            u.packs.Add(this);
        ...
        public void MoveTowards(Node u)
        {
            ...
            List<Node> path = this.dungeon.shortestpath(this.location, u); //take the shortest path between them
                                                                           //Logger.log("wants to move from " + path[0].id);
        ...
        /* OLD FLEE, in this version fleeing to different zone possible
		public bool flee()
		{
        ...
        public bool flee()
        {
            ...
            foreach (Node adjNode in adjacentNodes)
            {
                ...
                if (zoneLevel == currentLevel) //only move in the same zone
                {
